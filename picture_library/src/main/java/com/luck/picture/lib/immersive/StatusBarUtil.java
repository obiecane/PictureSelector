...
import com.luck.picture.lib.tools.DeviceUtils;
...
//            }
     ...
     * @param activity 窗口
     */
    public static int fullScreen(Activity activity) {
        if (activity == null) {
            return -1;
        }
        return fullScreen(activity.getWindow());
    }
     ...
     * @param window 窗口
     */
    public static int fullScreen(Window window) {
        if (window == null) {
            return -1;
        }
        int originSystemUiVisibility = window.getDecorView().getSystemUiVisibility();
    ...
    public static void showWindow(Activity activity, Window window, IWindowShower iWindowShower) {
        ...
        window.addFlags(WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE);
        iWindowShower.show(window);
        StatusBarUtil.syncSystemUiVisibility(activity, window);
        window.clearFlags(WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE);
    }
